CREATE DATABASE orders;
go;

USE orders;
go;

DROP TABLE review;
DROP TABLE shipment;
DROP TABLE productinventory;
DROP TABLE warehouse;
DROP TABLE orderproduct;
DROP TABLE incart;
DROP TABLE product;
DROP TABLE category;
DROP TABLE ordersummary;
DROP TABLE paymentmethod;
DROP TABLE customer;


CREATE TABLE customer (
    customerId          INT IDENTITY,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT IDENTITY,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE ordersummary (
    orderId             INT IDENTITY,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE category (
    categoryId          INT IDENTITY,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT IDENTITY,
    productName         VARCHAR(100),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        VARBINARY(MAX),
    productDesc         VARCHAR(8000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE warehouse (
    warehouseId         INT IDENTITY,
    warehouseName       VARCHAR(30),        
    PRIMARY KEY (warehouseId),
    
);

CREATE TABLE shipment (
    shipmentId          INT IDENTITY,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2), 
    categoryId          INT,
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (categoryId) REFERENCES category(categoryId), 
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
        
);

CREATE TABLE review (
    reviewId            INT IDENTITY,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Prebuilt Keyboards');
INSERT INTO category(categoryName) VALUES ('Foam');
INSERT INTO category(categoryName) VALUES ('Switches');
INSERT INTO category(categoryName) VALUES ('Keycaps');
INSERT INTO category(categoryName) VALUES ('Lube');
INSERT INTO category(categoryName) VALUES ('Keycap Puller');

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Double - Shot PBT Full Set Keycaps',4,'This white keycap set comes with double-shot black legends that provides visual and physical typing comfort. It can be used for both gaming and typing purposes.',58.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ MatterMint Series Custom Keycap Set',4,'Mechanical keyboards are cool, but not cool enough for your minty green keys. The mint theme keycaps on this GMK+ MatterMint Series Keycap Set are sure to make you the coolest on the block. These gorgeous keycaps are inspired by the popular mint theme with a hint of fresh white accents! With 129 pieces included, you’ll feel fresher than ever!',95.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ Bluebay Series Custom Keycap Set',4,'The Bluebay Series Custom Keycaps set is an eye-catching collection of custom keycaps. Each set includes intricately designed ABS keycaps as well as a unique keycap puller. The engraved design on the front of the ABS keycaps is complemented by a matching accent on the side. The contoured ABS keycaps have an engraved design on each keycap, ensuring a distinct appearance. The custom keycaps have a unique and retro design with accents of red, yellow, and two-tone blue. With 129 keycaps to choose from, you’ll love the new look of your keyboard.',95.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ Blue Haven Series Custom Keycap Set',4,'The GMK+ Blue Haven Series Custom Keycap Set is the perfect way to update the look of your custom keyboard. These keycaps are made from high-quality ABS plastic and feature a double-shot injection molding process that ensures long-lasting durability. The GMK+ Blue Haven Series custom keycap set is a gorgeous custom keycap set made of ABS material. With 172 keycaps to choose from, you’ll love the new look of your keyboard!',100.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ Geometric Series Keycap Set',4,'Have you ever felt as smart as Einstein? The GMK+ Geometric Series Keycap Set by GMK Keycaps is the ideal way to give your keyboard a unique and intelligent look. These keycaps are made of high-quality ABS plastic and have a unique geometric design that will make your keyboard stand out. The set includes all the keycaps you need to replace your existing keycaps, and the included keycap puller makes installation a breeze. With 127 keycaps included in this set, make the smart decision and order today!',100.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ Retro Cherry Keycap Set',4,'Boring keycaps are a thing of the past with the GMK+ Retro Cherry Keycap Set. These keycaps are made from a high-quality ABS plastic that is double-shot molded for durability and stability. The keycaps are also UV coated to resist fading and yellowing. The GMK+ Retro Cherry Keycap Set is available in a variety of colors and finishes, so you can find the perfect look for your keyboard. With 131 pieces to choose from, your keyboard will be thanking you!',100.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ Amethyst Series Keycap Set',4,'Amethyst is a semiprecious stone with healing properties that have been utilized for centuries. Amethyst is a deep purple gemstone that represents meditation, tranquility, and wisdom. The two-tone purple and cream-white caps of the crystals are fit for a king or queen! This GMK+ Amethyst Series Keycap Set features keycaps inspired by amethysts and crystals. This keycap set contains 138 pieces that are compatible with the vast majority of mechanical keyboards!',110.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('GMK+ Peachu Series Keycap Set',4,'Are you ready for a Peach and Summer look? The GMK+ Peachu Series Keycap Set is the perfect solution. The GMK+ Peachu Series Keycap Set features a smooth, creamy peach and apricot colorway that captures the summer vibes in one keyboard! This keycap set includes 141 creamy peach pop keycaps that are ideal for any mechanical keyboard.',110.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Keychron K2 Wireless Mechanical Keyboard (Version 2)',1,'K2 is a super tactile wireless or wired keyboard giving you all the keys and function you need while keeping it compact, with the largest battery seen in a mechanical keyboard.',80.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Keychron K8 Non-Backlight Wireless Mechanical Keyboard',1,'Keychron K8 Non-Backlight Tenkeyless Wireless Mechanical Keyboard has included keycaps for both Windows and macOS, and users can hotswap every switch in seconds with the hot-swappable version.',69.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gateron G Pro 2.0 Switch',3,'The G Pro 2.0 switches upgraded their mold from the original G Pro Series and offer an RGB SMD condenser, which is renowned for its dynamic lighting effect and tactile typing experience.',24.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Keychron Klube Lubricant',6,'Our carefully formulated K-Series lubricants are perfect for your personal keyboard tuning. We have a specially formulated switch lube as well as stabilizer lube.',10.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Keychron K2 Poron Foam Kit',2,'Foam for making your Keykron K2 sound better!',21.00);
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Keycap Puller for Mechanical Keyboard',5,'Design and realize of the dual purpose key pull and axle pull.',9.00);



INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (1, 1, 1, 58, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (2, 1, 1, 95, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (3, 1, 1, 95, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (4, 1, 1, 100, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (5, 1, 1, 100, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (6, 1, 1, 100, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (7, 1, 1, 110, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (8, 1, 1, 110, 4);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (9, 1, 1, 80, 1);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (10, 1, 1, 69, 1);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (11, 1, 1, 24, 3);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (12, 1, 1, 10, 6);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (13, 1, 1, 21, 2);
INSERT INTO productInventory(productId, warehouseId, quantity, price, categoryId) VALUES (14, 1, 1, 9, 5);




INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');

-- Order 1 can be shipped as have enough inventory
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (1, '2019-10-15 10:25:55', 101)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 9, 1, 80.00)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 2, 21.00);


DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-16 18:00:00', 69.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 10, 5, 69.00);

-- Order 3 cannot be shipped as do not have enough inventory for item 7
DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (3, '2019-10-15 3:30:22', 31.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 12, 2, 10)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 13, 3, 21);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (2, '2019-10-17 05:45:11', 399.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 4, 100)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 7, 3, 110)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 4, 3, 100)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 9, 2, 80)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 14, 4, 9);

DECLARE @orderId int
INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (5, '2019-10-15 10:25:55', 300.00)
SELECT @orderId = @@IDENTITY
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 5, 5, 100)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 6, 2, 100)
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@orderId, 4, 1, 100);

-- New SQL DDL for lab 8
UPDATE Product SET productImageURL = 'img/img1.jpg' WHERE ProductId = 1;
UPDATE Product SET productImageURL = 'img/img2.jpg' WHERE ProductId = 2;
UPDATE Product SET productImageURL = 'img/img3.jpg' WHERE ProductId = 3;
UPDATE Product SET productImageURL = 'img/img4.jpg' WHERE ProductId = 4;
UPDATE Product SET productImageURL = 'img/img5.jpg' WHERE ProductId = 5;
UPDATE Product SET productImageURL = 'img/img6.jpg' WHERE ProductId = 6;
UPDATE Product SET productImageURL = 'img/img7.jpg' WHERE ProductId = 7;
UPDATE Product SET productImageURL = 'img/img8.jpg' WHERE ProductId = 8;
UPDATE Product SET productImageURL = 'img/img9.jpg' WHERE ProductId = 9;
UPDATE Product SET productImageURL = 'img/img10.jpg' WHERE ProductId = 10;
UPDATE Product SET productImageURL = 'img/img11.jpg' WHERE ProductId = 11;
UPDATE Product SET productImageURL = 'img/img12.jpg' WHERE ProductId = 12;
UPDATE Product SET productImageURL = 'img/img13.jpg' WHERE ProductId = 13;
UPDATE Product SET productImageURL = 'img/img14.jpg' WHERE ProductId = 14;



-- Loads image data for product 1
UPDATE Product SET productImage =  WHERE ProductId = 1;